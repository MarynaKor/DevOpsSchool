...
Ex1 : 
       Build a Conatiner for Prometheus, which can be reached through http://localhost:9090 and shows your applications metrics.
...
Ex2 :
       Create your own Metric, which shows the functionality of the counter service application. (Suggestion: Create a metric for the iteration in the updating process).
...
Ex3 : 
       Based on the previous metric, create a rule in Prometheus, which shows that the iteration is only running through as many times as there are active projects.
...
Ex4 :
       ...
       Baue einen weiteren Container für den Alertmanager.

Ex5: 
       Configure the Alertmanager so that is sends an Alert(it does not matter what channel you choose) whenever the rule is firing.
       Erstelle für den Alertmanager eine Konfiguration mit der Alerts versendet werden, sobald eine Regel feuert.
